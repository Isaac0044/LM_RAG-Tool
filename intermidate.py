...
def summary(messages):
    ...
    combined = cc.convert(sysmessage) + cc.convert(messages)
    ...
    if st.session_state['messages']:
        ...
        with open(filename, 'w', encoding='UTF-8') as f:
            for message in st.session_state['messages']:
                ...
                f.write(f"{message['role']}: {cc.convert(encoded_content)}\n")
...
def load_chat(file_path):
    ...
    with open(file_path, 'r', encoding='UTF-8') as file:
        for line in file.readlines():
            ...
            cc.convert(st.session_state['messages']).append({'role': role, 'content': decoded_content})
...
def main():
    ...
    if user_input:
        ...
        with st.chat_message("assistant"):
            st.write_stream(cc.convert(response_generator(response["content"])))    
